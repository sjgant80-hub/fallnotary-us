# Fallnotary · sovereign signing management + US law engine

> Single-file signing management + US law research for 1-10 person State Bar-regulated attorney firms. One HTML file, runs entirely in your browser, no server, no telemetry, no signup. Client and signing data lives in IndexedDB on the device you opened it on.

**Anchor of the law bundle** (prime 743). Sibling tools in the same bundle: `fallnotary-usonboard` (CDD/AML/conflict register · 751), `fallnotary-uspaper` (engagement + advice letters + precedents · 757), `fallnotary-uspractice` (State Bar Accounts Rules + time + billing · 761). They sync over `BroadcastChannel('fall-law')` and signal estate-wide on `BroadcastChannel('fall-signal')`.

---

## For attorneys

You're a 1-10 person firm. You want signing management, conflict-checking, file retention, critical-date flagging, and a US law research engine — without paying $80/user/month and without sending privileged data to someone's cloud.
...
- **Multi-signing sidebar** with search by title / signing ref / opposing party / client; filter by practice area, responsible attorney, status, risk, due date. Critical dates (limitation / hearing / next step) flag at 28d / 14d / 7d / today.
- **Signing detail** — seven tabs: Overview, Retainer & Fees, Conflict & CDD, Timeline & Dates, Advice, Documents, Time / Fees. Every field edit drops a hashed audit entry.
- **12 practice areas** — civil litigation, conveyancing, family, crime, wills & probate, employment, commercial, immigration, landlord & tenant, personal injury, clinical negligence, other. Each surfaces relevant statutes + matching weaves when you open a signing.
- **30 strategic weaves** — named patterns with archetype / move / authorities / opposition / counter / why-it-wins / example. Covers Limitation Act, Bolam/Bolitho, Montgomery, CFAs, Part 36, TUPE, Inheritance Act, Section 21, Equality Act 2010, Companies Act derivative + unfair prejudice, Norwich Pharmacal, Attorneys Act assessment, and more. Apply a weave to a signing to log strategic position.
- **US Law corpus** — Attorneys Act 1974, Notary Services Act 2007, Limitation Act 1980, CPR, Family Law Act 1996, Land Registration Act 2002, Wills Act 1837, Inheritance Act 1975, Equality Act 2010, ERA 1996, Companies Act 2006, Housing Act 1988, Children Act 1989, PACE 1984, MCA 1973.
- **Advice issuance** — write opinions in the signing, sign with sha256 + adviser id + timestamp, retained 6 years per State Bar 13.5.
- **Conflict scan** — on every new signing, scan local clients/signings AND broadcast `conflict.check.request` on `fall-law` for sibling tools to respond.
- **Q & A** — 14 one-tap T0 chips (PI limitation, CFA vs DBA, State Bar conduct, Bolam, conveyancing protocol, wasted costs, Inheritance Act, s.21 vs s.8, Equality Act, ET time limits, Norwich Pharmacal, CPR 31, without prejudice, privilege types). T3 cloud cascade (Anthropic → OpenAI → Gemini → OpenRouter) using your own API key — context includes the active signing and client.
- **P3 audit chain** — every state change appended with prevHash + docHash + reasoning + adviser/client/signing ids. Exportable as JSON. 6-year retention.
- **Demo signing** seeded on first launch — overwrite or purge from Firm → Settings.

> **Disclaimer.** Fallnotary is a tool for State Bar-regulated US attorneys. It assists with signing management, CDD, document generation, and State Bar Accounts Rules tracking. It is **not regulatory submission or notary opinion software**. The firm's COLP / COFA remain responsible. Sovereign — client data never leaves the device unless you export it.
...
- **IDB stores** (9): `firms`, `advisers`, `clients`, `signings`, `advice`, `corpus`, `weaves`, `audit`, `settings`
- **Record schemas** conform to `LAW-BUNDLE-SHARED-SCHEMA.md` (Signing / NotaryClient / NotaryAdviser / Firm)
- **Audit chain** — P3 extended: `{i, ts, tool, adviserId, clientId, signingId, action, reasoning, configVersion, prevHash, docHash, payload}`. Cap 100k entries.
- **Mesh** — `BroadcastChannel('fall-law')` for signing/client/adviser/firm sync + conflict.check.request/response. `BroadcastChannel('fall-signal')` for estate hello/ping.
...
```js
// emitted
{v:1, type:'signing.created'|'signing.updated'|'signing.closed'|'signing.reopened',
 source:'fallnotary-us', ts, payload:Signing}
...
{v:1, type:'sync.request'|'sync.snapshot', source, ts, payload:{firm, advisers[], clients[], signings[]}}
```
...
Append to `PRACTICE_AREAS` — `{id, name, statutes[]}`. Statutes show in signing overview when that area is selected. Weave-match is by lowercase substring on archetype + move + authorities, so name your weave authorities accordingly.
...
FALLNOTARY-US.WEAVES.length // 30
FALLNOTARY-US.T0_RULES.length // 14
FALLNOTARY-US.state.signings.length
FALLNOTARY-US.scanConflicts({clientName:'Patel'}) // exercise conflict scan
```
...
- **Discoverable** — indexed on the FallMarket catalog at [sjgant80-hub.github.io/fallmarket/listing.html?id=fallnotary-us](https://sjgant80-hub.github.io/fallmarket/listing.html?id=fallnotary-us)
...
- **Companion SDK** (`npm install @ai-native-solutions/fallnotary-us-sdk`) — programmatic JavaScript API for direct integration.
- **Companion MCP server** (`claude mcp add fallnotary-us -- npx -y @ai-native-solutions/fallnotary-us-mcp`) — Model Context Protocol server that any Claude/Cursor/Codex client can invoke as a tool.
- **Companion HTTP API** (`docker run --rm ghcr.io/sjgant80-hub/fallnotary-us-api`) — containerized REST endpoint for language-agnostic access.
...
- **Registry entry**: https://sjgant80-hub.github.io/fallmarket/listing.html?id=fallnotary-us
- **Live URL**: https://sjgant80-hub.github.io/fallnotary-us/
- **Source**: https://github.com/sjgant80-hub/fallnotary-us
